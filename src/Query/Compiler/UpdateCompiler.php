<?php

declare(strict_types=1);

namespace Diogodg\Neoorm\Query\Compiler;

use Diogodg\Neoorm\Dialect\Dialect;
use Diogodg\Neoorm\Query\Exception\QueryException;
use Diogodg\Neoorm\Query\State\UpdateState;

final class UpdateCompiler
{
    public function __construct(private readonly ExpressionCompiler $expressions = new ExpressionCompiler())
    {
    }

    public function compile(UpdateState $state, Dialect $dialect, ?BindCollector $binds = null): CompiledQuery
    {
        if ($state->assignments === []) {
            throw new QueryException("UPDATE em {$state->table->name} sem nenhuma coluna para atualizar.");
        }

        if ($state->where === null && !$state->allowFullTableScan) {
            throw new QueryException(
                "UPDATE em {$state->table->name} sem WHERE reescreveria a tabela inteira. Se é "
                . 'mesmo isso, declare com allowFullTableScan().',
            );
        }

        $binds ??= new BindCollector();

        $assignments = [];

        foreach ($state->assignments as $column => $value) {
            $assignments[] = $dialect->quoteIdentifier($column)
                . ' = ' . $this->expressions->compile($value, $dialect, $binds);
        }

        $sql = 'UPDATE ' . $dialect->quoteIdentifier($state->table->name)
            . ' SET ' . implode(', ', $assignments);

        if ($state->where !== null) {
            $sql .= ' WHERE ' . $this->expressions->compile($state->where, $dialect, $binds);
        }

        if ($state->returning !== null) {
            if (!$dialect->supportsReturningOnModify()) {
                throw new QueryException(
                    "O dialeto {$dialect->name()} não tem RETURNING no UPDATE. Releia as linhas "
                    . 'com um SELECT depois, dentro da mesma transação.',
                );
            }

            $sql .= ' RETURNING ' . implode(', ', array_map(
                static fn (string $column): string => $dialect->quoteIdentifier($column),
                $state->returning,
            ));
        }

        return new CompiledQuery($sql, $binds->all());
    }
}
