<?php

namespace Tests\App\Models;

use Diogodg\Neoorm\Abstract\Model;
use Diogodg\Neoorm\Migrations\Col;
use Diogodg\Neoorm\Migrations\Table;
use Tests\Generated\ScheduleEmployeeTable;

/**
 * @extends Model<ScheduleEmployeeTable>
 */
class ScheduleEmployee extends Model
{
    public const table = "schedule_employee";

    public static function table(): Table
    {
        return Table::make(self::table, comment: "Schedule Employee associations")
            ->columns([
                'id' => Col::id()->comment("Association ID"),
                'schedule_id' => Col::int()->notNull()->references(Schedule::class)->comment("Schedule ID"),
                'employee_id' => Col::int()->notNull()->references(Employee::class)->comment("Employee ID"),
            ]);
    }
}
