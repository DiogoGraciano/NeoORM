-- Consultas do dialeto mysql, geradas por CompilerGoldenTest.
...
SELECT `users`.* FROM `users` WHERE LOWER(`users`.`name`) LIKE LOWER(:p0);
--   :p0 = 'd%' (PDO type 2)
...
-- [select_order_nulls]
SELECT `users`.* FROM `users` ORDER BY `users`.`age` IS NULL ASC, `users`.`age` DESC, `users`.`name` ASC;
...
-- [select_offset_only]
SELECT `users`.* FROM `users` LIMIT 18446744073709551615 OFFSET :p0;
--   :p0 = 20 (PDO type 1)
...
-- [insert_returning]
-- recusado: O dialeto mysql não tem RETURNING no INSERT. Quem precisa da linha gravada de volta usa returningOne(), que faz o SELECT complementar por lastInsertId(); em lote, só execute().

-- [update_returning]
-- recusado: O dialeto mysql não tem RETURNING no UPDATE. Releia as linhas com um SELECT depois, dentro da mesma transação.

-- [delete_returning]
-- recusado: O dialeto mysql não tem RETURNING no DELETE. Leia as linhas antes de apagar, dentro da mesma transação.
